...
namespace Maydear.Extensions
{
    ...
    public static class StringFilterExtension
    {
        ...
        {
            var text1 = data.Replace("%20", " ");
            text1 = Regex.Replace(text1, @"\s", " ");
            var text2 = "select |insert |delete from |count\\(|drop table|update |truncate |asc\\(|mid\\(|char\\(|xp_cmdshell|exec master|net localgroup administrators|:|net user|\"|\\'| or ";
        ...
        public static bool IsChineseIdNumber18(this string data)
        {
            ...
            if (!DateTime.TryParse($"{data.Substring(6, 4)}-{data.Substring(10, 2)}-{data.Substring(12, 2)}", out var date))
                return false;
